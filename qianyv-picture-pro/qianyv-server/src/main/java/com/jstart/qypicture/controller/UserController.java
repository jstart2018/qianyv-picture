...
import com.jstart.qypicture.auth.SystemRole;
...
public class UserController {
    ...
    public Page<User> page(@RequestBody UserQueryDTO userQueryDTO) {
        ...
        Integer current = userQueryDTO.getPageNum();
    ...
    public Result<String> changeRole(Long userId) {
        ...
        if (user.getRole().equals(SystemRole.BOSS.getKey())){
            throw new BusinessException(ResultEnum.PARAMS_ERROR, "不能修改老板角色");
        } else if (user.getRole().equals(SystemRole.ADMIN.getKey())) {
            updateUser.setRole(SystemRole.user.getKey());
        } else {
            updateUser.setRole(SystemRole.ADMIN.getKey());
        }
