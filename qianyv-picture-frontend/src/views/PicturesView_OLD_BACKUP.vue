...
const fetchPictures = async (isLoadMore = false) => {
  ...
  // 如果不是加载更多且有图片，先触发旧图退出动画（向左移动+淡出）
...
const handlePictureTypeChange = (type: number) => {
  ...
  // 重置分页并请求
  // 注意：fetchPictures 会先淡出旧图，然后切换 displayPictureType，最后显示新图
  current.value = 1
  fetchPictures()
}
...
<template>
  <div class="pictures-view">
    ...
    <div
      ...
      :class="{ animate: animatePictureType, 'fade-out': isTransitioning }"
    >
      <!-- 加载状态 -->
...
/* 旧图片：向左移动并快速淡出（无缩放，加快淡出避免布局切换可见） */
...
/* 悬停时整个卡片放大 */
.picture-card:hover {
  transform: scale(1.05); /* 整个卡片放大到 1.05 倍 */
  box-shadow:
    8px 8px 20px rgba(0, 0, 0, 0.4),
    15px 15px 45px rgba(0, 0, 0, 0.3);
  z-index: 10; /* 提升层级，避免被其他卡片遮挡 */
}
...
/* 底部按钮区域 - 无白底卡片，按钮直接在图片上 */
.picture-footer {
  position: absolute;
  bottom: 0;
  left: 0;
  right: 0;
  padding: 20% 10%; /* 增大底部边距从 12px 到 20px */
  display: flex;
  justify-content: center;
  align-items: center;
...
.picture-goto-btn {
  ...
  align-items: center;
  gap: 8px;
  padding: 12px 80px; /* 增大水平长度从 32px 到 80px */
  ...
  cursor: pointer;
  transition: all 0.3s ease;
  box-shadow: 0 4px 16px rgba(100, 100, 100, 0.4); /* 增强阴影 */
...
.grid-container {
  ...
  row-gap: 23px; /* 从 18px 减少到 14px，减小垂直间距 */
